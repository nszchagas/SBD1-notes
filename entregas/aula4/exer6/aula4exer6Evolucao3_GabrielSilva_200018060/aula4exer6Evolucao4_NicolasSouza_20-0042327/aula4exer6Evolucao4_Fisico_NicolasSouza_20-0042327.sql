...
DROP VIEW V_DADOS_VEICULOS;
CREATE VIEW V_DADOS_VEICULOS (Proprietario, Placa, Cor, Ano, Chassi, Categoria) AS 
	SELECT p.cpf, v.placa, v.cor, v.anoFabricacao, v.chassi, c.dsCategoria 
		FROM VEICULO v JOIN CATEGORIA c JOIN PROPRIETARIO p
			ON v.idCategoria = c.idCategoria AND p.cpf = v.cpf;		